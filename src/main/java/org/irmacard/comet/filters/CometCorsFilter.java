 ...
 */
package org.irmacard.comet.filters;
...
/**
 * <p>
 * A {@link CometFilter} that enable client-side cross-origin requests by
 ...
 */
public final class CometCorsFilter implements CometFilter {

    private static final Log log = LogFactory.getLog(CometCorsFilter.class);
	...
	public void doFilterEvent(CometEvent event, CometFilterChain filterChain)
        ...
        CometCorsFilter.CORSRequestType requestType = checkRequestType(request);

        // Adds CORS specific attributes to request.
        if (decorateRequest) {
            CometCorsFilter.decorateCORSProperties(request, requestType);
        }
        switch (requestType) {
        case SIMPLE:
            // Handles a Simple CORS request.
            this.handleSimpleCORS(event, filterChain);
            break;
        case ACTUAL:
        ...
        case PRE_FLIGHT:
            ...
            this.handlePreflightCORS(event, filterChain);
            break;
        case NOT_CORS:
            // Handles a Normal request that is not a cross-origin request.
            this.handleNonCORS(event, filterChain);
            break;
        default:
            // Handles a CORS request that violates specification.
            this.handleInvalidCORS(event, filterChain);
            break;
        }
	}
     ...
     */
    protected void handleSimpleCORS(CometEvent event, CometFilterChain filterChain)
            throws IOException, ServletException {
        ...
        CometCorsFilter.CORSRequestType requestType = checkRequestType(request);
        if (!(requestType == CometCorsFilter.CORSRequestType.SIMPLE ||
                requestType == CometCorsFilter.CORSRequestType.ACTUAL)) {
            throw new IllegalArgumentException(
                    sm.getString("corsFilter.wrongType2",
                            CometCorsFilter.CORSRequestType.SIMPLE,
                            CometCorsFilter.CORSRequestType.ACTUAL));
        }

        final String origin = request
                .getHeader(CometCorsFilter.REQUEST_HEADER_ORIGIN);
        final String method = request.getMethod();

        // Section 6.1.2
        if (!isOriginAllowed(origin)) {
            handleInvalidCORS(event, filterChain);
            return;
        }
        ...
        if (anyOriginAllowed && !supportsCredentials) {
            ...
            addHeader(response, 
                    CometCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN,
                    "*");
        } else {
        ...
        if (supportsCredentials) {
            addHeader(response, 
                    CometCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS,
                    "true");
        }
        ...
        if ((exposedHeaders != null) && (exposedHeaders.size() > 0)) {
            ...
            addHeader(response, 
                    CometCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
                    exposedHeadersString);
        }

        // Forward the request down the filter chain.
        filterChain.doFilterEvent(event);
    }
     ...
     * @throws ServletException
     */
    protected void handlePreflightCORS(CometEvent event, final CometFilterChain filterChain)
            throws IOException, ServletException {
        ...
        String accessControlRequestMethod = request.getHeader(
                CometCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
        ...
        String accessControlRequestHeadersHeader = request.getHeader(
                CometCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
        ...
        if (preflightMaxAge > 0) {
            addHeader(response, 
                    CometCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE,
                    String.valueOf(preflightMaxAge));
        }

        // Section 6.2.9
        addHeader(response, 
                CometCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS,
                accessControlRequestMethod);

        // Section 6.2.10
        if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
            addHeader(response, 
                    CometCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS,
     ...
     * @throws ServletException
     */
    private void handleNonCORS(CometEvent event, CometFilterChain filterChain)
            throws IOException, ServletException {
        // Let request pass.
        filterChain.doFilterEvent(event);
    }
     ...
     *            The {@link FilterChain} object.
     */
    private void handleInvalidCORS(CometEvent event, CometFilterChain filterChain) {

    	// Get request and response from event
        HttpServletRequest request = event.getHttpServletRequest();
        HttpServletResponse response = event.getHttpServletResponse();

        String origin = request.getHeader(CometCorsFilter.REQUEST_HEADER_ORIGIN);
    ...
    /**
     * Helper function to prevent adding duplicate headers.
     */
    private void addHeader(HttpServletResponse response, String name, String value) {
    	if (!response.getHeaders(name).contains(value)) {
    ...
    protected static void decorateCORSProperties(
        ...
        case SIMPLE:
            request.setAttribute(
                    CometCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
                    Boolean.TRUE);
            request.setAttribute(CometCorsFilter.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
                    request.getHeader(CometCorsFilter.REQUEST_HEADER_ORIGIN));
            request.setAttribute(
                    CometCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE,
        ...
        case PRE_FLIGHT:
            ...
            request.setAttribute(
                    CometCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS, headers);
            break;
        case NOT_CORS:
            request.setAttribute(
                    CometCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
